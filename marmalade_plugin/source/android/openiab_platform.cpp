...
OpenIabSkuDetails* getSkuDetails_platform(const char* sku)
{
    JNIEnv* env = s3eEdkJNIGetEnv();
    jstring sku_jstr = env->NewStringUTF(sku);
    return (OpenIabSkuDetails*) env->CallObjectMethod(g_Obj, g_getSkuDetails, sku_jstr);
}
...
{
    jclass purchase = env->FindClass("oorg/onepf/oms/appstore/googleUtils/Purchase");
